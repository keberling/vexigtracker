...
export async function lookupPublicPost(postUrl) {
  const clean = String(postUrl || "").trim().split(/[?#]/)[0];
  if (!/^https:\/\/(www\.)?instagram\.com\/(p|reel|reels)\//i.test(clean)) {
    throw new InstagramApiError("Paste a full Instagram post or reel URL (instagram.com/p/… or /reel/…).", 400);
  }
  if (!IG_APP_ID || !IG_APP_SECRET) {
    throw new InstagramApiError("Set IG_APP_ID and IG_APP_SECRET to look up a public post URL.", 400);
  }
  const appToken = `${IG_APP_ID}|${IG_APP_SECRET}`;
  let lastErr;
  for (const host of ["graph.instagram.com", "graph.facebook.com"]) {
    try {
      const data = await graphGet(host, "/instagram_oembed", {
        url: clean,
        access_token: appToken,
      });
      const fromUrl = String(data.author_url || "").match(/instagram\.com\/([^/?#]+)/i)?.[1];
      const username = String(fromUrl || data.author_name || "")
        .replace(/^@/, "")
        .toLowerCase();
      return {
        id: `oembed:${clean}`,
        username,
        caption: data.title || "",
        mediaType: /\/reel/i.test(clean) ? "VIDEO" : "IMAGE",
        permalink: clean,
  ...
  throw lastErr || new InstagramApiError("Could not read that post. If the personal account is private, the API cannot see it.", 400);
}
